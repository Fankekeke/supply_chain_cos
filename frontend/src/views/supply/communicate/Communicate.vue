...
<!--        <a-button type="primary" ghost @click="add">新增</a-button>-->
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          <a-icon type="setting" theme="twoTone" twoToneColor="#4a9ff5" @click="edit(record)" title="修 改" v-if="record.status == 0"></a-icon>
        </template>
      </a-table>
    </div>
    <bulletin-add
...
import BulletinAdd from './CommunicateAdd.vue'
import BulletinEdit from './CommunicateEdit.vue'
...
export default {
    ...
    columns () {
      return [{
        ...
        customRender: (text, row, index) => {
          switch (text) {
            case '0':
              return <a-tag>未回复</a-tag>
            case '1':
              return <a-tag>已回复</a-tag>
            default:
              return '- -'
          }
        }
  ...
  methods: {
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$delete('/cos/reply-price-info/' + ids).then(() => {
    ...
    fetch (params = {}) {
      ...
      this.$get('/cos/reply-price-info/page', {
        ...params
      }).then((r) => {
