...
import cc.mrbird.febs.cos.dao.OrderRawInfoMapper;
...
public class OrderRawInfoServiceImpl extends ServiceImpl<OrderRawInfoMapper, OrderRawInfo> implements IOrderRawInfoService {
     ...
     */
    @Override
    public IPage<LinkedHashMap<String, Object>> selectOrderPage(Page<OrderRawInfo> page, OrderRawInfo orderInfo) {
        ...
        return new LinkedHashMap<String, Object>() {
            {
                ...
                put("orderDetail", baseMapper.selectOne(Wrappers.<OrderRawInfo>lambdaQuery().eq(OrderRawInfo::getCode, code)));
            }
        };
    }
    ...
    public boolean saveOrder(OrderRawInfo orderInfo) {
        // 添加订单编号
    ...
    public LinkedHashMap<String, Object> selectStatisticsByMonth(String year, String month) {
        // 收入 【订单结余】
        List<OrderRawInfo> orderInfoList = baseMapper.selectOrderInfoByDate(year, month);
        // 支出【采收入库+员工薪资】
        ...
        return new LinkedHashMap<String, Object>() {
            {
                ...
                put("orderTotalPrice", orderInfoList.stream().map(OrderRawInfo::getTotalPrice).reduce(BigDecimal.ZERO,BigDecimal::add));
    ...
    public LinkedHashMap<String, Object> selectMaterialTypeRate(String year, String month) {
        ...
        List<OrderRawInfo> orderInfoList = baseMapper.selectOrderInfoByDate(year, month);
        // 获取订单详情
        List<String> orderNumberList = orderInfoList.stream().map(OrderRawInfo::getCode).collect(Collectors.toList());
